<?php
	...
	if($admin!=''){
		...
		if($count>0){
			...
			if(session_is_registered($row[1]) && session_is_registered($row[2]) && $row[3]="ADM"){
				$query = mysql_query("select * from pengarang");
			}else {
	...
	echo "Daftar Pengarang</center><br><br>";
	echo "<a href=index.php?pg=formPengarang&admin=$admin>add</a>";  
...
echo "<a href=index.php?pg=formPengarang&admin=$admin&idPengarang=$p[0]>edit</a>";
echo "</td><td align=center>";
echo "<a href=index.php?pg=deletePengarang&admin=$admin&idPengarang=$p[0]>delete</a>";
echo "</td></tr>";
